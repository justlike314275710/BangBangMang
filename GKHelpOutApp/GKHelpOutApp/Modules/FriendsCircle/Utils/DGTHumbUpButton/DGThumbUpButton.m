...
- (instancetype) initWithFrame: (CGRect)frame isPress: (BOOL)press type: (DGThumbUpButtonType)type {
    ...
    isSelected = press;
    if (isSelected) {
        [self setImage: [UIImage imageNamed: @"已点赞icon"] forState: UIControlStateNormal];
    } else {
        [self setImage: [UIImage imageNamed: @"未点赞icon"] forState: UIControlStateNormal];
    }
...
- (void) popOutsideWithDuration: (NSTimeInterval) duringTime {
    ...
    [UIView animateKeyframesWithDuration: duringTime delay: 0 options: 0 animations: ^{
        [weakSelf setImage: [UIImage imageNamed: @"已点赞icon"] forState: UIControlStateNormal];
...
- (void) popInsideWithDuration: (NSTimeInterval) duringTime {
    ...
    [UIView animateKeyframesWithDuration: duringTime delay: 0 options: 0 animations: ^{
        [weakSelf setImage: [UIImage imageNamed: @"未点赞icon"] forState: UIControlStateNormal];
        [UIView addKeyframeWithRelativeStartTime: 0
