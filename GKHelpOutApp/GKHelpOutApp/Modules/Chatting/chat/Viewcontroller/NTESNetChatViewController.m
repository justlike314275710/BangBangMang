...
- (void)doStartByCaller
{
    ...
    self.option = [[NIMNetCallOption alloc] init];
   
    NSDictionary*dic=@{@"legalAdviceId":self.orderID,@"videoDuration":self.videoDuration};
    NSError *parseError = nil;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:&parseError];
    NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    self.option.extendMessage =jsonString;
    self.option.apnsContent = [NSString stringWithFormat:@"%@请求", self.callInfo.callType == NIMNetCallTypeAudio ? @"网络通话" : @"视频聊天"];
    self.option.apnsSound = @"video_chat_tip_receiver.aac";
    [self fillUserSetting:self.option];
    self.option.videoCaptureParam.startWithCameraOn = (self.callInfo.callType == NIMNetCallTypeVideo);

    __weak typeof(self) wself = self;

    [[NIMAVChatSDK sharedSDK].netCallManager start:callees type:wself.callInfo.callType option:self.option completion:^(NSError *error, UInt64 callID) {
        if (!error && wself) {
