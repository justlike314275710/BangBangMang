...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        [self setBackgroundColor:UIColorFromRGBA(71, 70, 73, 1)];
        UIView *selectedView = [[UIView alloc] init];
        [selectedView setBackgroundColor: UIColorFromRGBA(65, 64, 67, 1.0)];
...
- (void)layoutSubviews
{
    ...
    self.addSeparator(ZZSeparatorPositionBottom).beginAt(15).endAt(-15).color([UIColor colorWithWhite:0.5 alpha:0.3]);
}
