...
@property (nonatomic, assign) int longTimes;
...
- (void)waitToCallInterface{
    ...
    NIMKitInfo *info = [[NIMKit sharedKit] infoByUser:self.callInfo.caller option:nil];
    NSString*nickName=info.showName;
    self.connectingLabel.text = [nickName stringByAppendingString:@"的来电"];
...
- (void)onNTESTimerFired:(NTESTimerHolder *)holder{
    ...
    NSLog(@"NTEVIDEO %@",self.videoDuration);
//[self.videoDuration intValue]*60
    if ( (int) duration >[self.videoDuration intValue]*60&&self.callInfo.startTime>0) {
        if (_times<1) {
            [self hangup];
            [PSTipsView showTips:@"您的通话时长已用完!"];
        }
        _times++;
    }
    
    if ([self.videoDuration intValue]*60-(int)duration<60&&self.callInfo.startTime>0){
        if (_longTimes<1) {
            [PSTipsView showTips:@"您的通话时长已不足一分钟!"];
        }
  }
}
